...
bool checkWord(const char * guess, char** result);
...
help()
{
  printf("Wordle is a simple game:" EOL "Guess the %i letter word within %i tries" EOL,
...
gameLoop() {
  ...
  // you can control pr change the value of MAX_TRIES
...
#define Felix_HACK
      ...
      if (Felix_Hack == 1)
  {
    printf("Initial Guess: earth\n");
    if (checkWord("earth", &result))
      {
        //removeAlpha("earth");
        printf("\t%s\n", alpha);
      }
    if (strcmp("earth", word) == 0)
      {
...
//create temporary word
    char* tempWord = (char*)calloc(6,sizeof(char));
    for(int i=0; i<5; i++){
      tempWord[i] = '?';
    }
    tempWord[5] = '\0';
    //while (!isWord(tempWord) || !hasWord(tempWord)){
    for(int i=0; i<5; i++){
      // if ((result)[i] == 'o') {
      //     for(int j=0; j<5; j++){
      //          if(j != i && (result)[j] != '#'){
      ...
      //      tempWord[i] = alpha[k];
      //      //tempAlpha[0] = alpha[k];
      //      //removeAlpha(tempAlpha);
      //      break;
      //     }
      //   }
      // }
      if ((result)[i] == '_' || result[i] == 'o') {
        for(int k=0; k<strlen(alpha); k++){
          if(alpha[k] != '_'){
           tempWord[i] = alpha[k];
           //tempAlpha[0] = alpha[k];
    ...
    //}
    printf("%s\n", tempWord);
    if(checkWord(tempWord, &result)){
        removeAlpha(tempWord);
        printf("\t%s\n", alpha);      
    }
    if (strcmp(tempWord, word) == 0)
      {
        printf("Solution Worked! The word was %s\n", word);
        free(tempWord);
        free(result);
        exit(0);
    }
